...
| `L` | Load 200 more revisions while preserving selection and scroll |
...
To edit a full description, select a revision and choose **Edit description in editor** from the Space menu. JJ opens the editor configured by `ui.editor` (or its usual environment fallbacks) with the complete description, including multiple paragraphs. Save and close to apply it; exit without changes to keep the description, or use your editor's failure/abort command to cancel. The app resumes and refreshes when JJ exits, keeping the selected change and revset. Press `d` for the existing quick single-line input; it directs multiline descriptions to the menu so they cannot be accidentally flattened.
...
The initial revision list loads 200 revisions. Press `L` when the revset bar offers **load 200 more** to expand the graph, preserving selection and scroll; refresh retains the expanded limit, and changing the revset resets it. Return from temporary context views with `Ctrl+O` before loading more. Destination pickers cover all revisions: press `/` to filter descriptions, bookmarks, or full change/commit IDs, use arrows to choose, and Enter to select. Escape clears the filter before returning to the form. Inline `R`/`S` also offer `/` to open a searchable destination picker and `L` to expand the graph. Search covers the full active revset, including full multiline descriptions and local and remote bookmark names. Text matching ignores case; change and commit IDs match by prefix. Typing previews the first match. Enter keeps the query for next and previous navigation; Escape restores the previous search and selection. Clearing an accepted search keeps the selection and revset. Matching revisions have a `*` marker. Distant targets open a temporary view of up to 40 revisions, including immediate parents and children. A `+` marks context outside the active revset. `Ctrl+O` restores the original selection and scroll position, including after repeated jumps. Search and navigation leave repository state unchanged. Operation history starts with 50 entries and offers **Load older operations**. History refreshes automatically when the terminal reports focus returning after commands in another terminal. The active revset, selected change, and scroll positions are retained; an open status view also reloads. Open prompts keep their drafts and defer refresh until they close. Returning focus invalidates an operation review: press `p` to review again before applying. Temporary navigation views defer refresh until `Ctrl+O` returns to the active revset. Terminals without focus reporting can still refresh with `r`. Existing multiline descriptions can be preserved during squash and edited through the configured external editor.
